	...
	<b-row class="h-100" style="height: 100vh;">
		<b-col cols="12">
			<b-card header-class="d-flex justify-content-between align-items-center" body-class=" h-100 px-0 pt-0 pb-2">
				<template #header>
					<div>
						<h4 class="card-title">{{ $t('competences') }}</h4>
						<h6 class="card-subtitle mb-2 text-body-secondary">{{ $t('Vos compétences professionnelles') }}</h6>
					</div>
				...
				<div class="p-0">
					...
					<table class="table align-items-center justify-content-center mb-0">
						<thead>
						  	<tr>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">{{ $t('Compétence') }}</th>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 text-center">{{ $t('Niveau') }}</th>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">{{ $t('Statut') }}</th>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2" style="width: 35%">{{ $t('Description') }}</th>
								<th></th>
						  	</tr>
						</thead>
					...
					<Pagination
						:links="props.competences.links"
						:prev="props.competences.prev_page_url"
						:next="props.competences.next_page_url"
						:total="props.competences.total"
						v-model="per_page"
						:from="props.competences.from"
						:to="props.competences.to"
					/>
				  </div>
			</b-card>
		</b-col>
...
const search    = ref('')
const per_page  = ref(props.competences.per_page ?? 5)

const items = computed(() => props.competences.data)
...
function removeItem(item) {
	$alert.confirm($t('Voulez-vous vraiment supprimer cette compétence ?'), () => {
		Inertia.post(route("competences.delete", item.id), {
			onSuccess: ({ props }) => {
...
const handleSearch = _.throttle(function() {
	const query = { 
  	...
  	Inertia.get(route('competences.index', query), {}, {
    	replace: true,
    	preserveState: true
  	})
}, 500)
